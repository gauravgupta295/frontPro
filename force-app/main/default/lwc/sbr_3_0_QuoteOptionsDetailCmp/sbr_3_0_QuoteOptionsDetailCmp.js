...
export default class sbr_3_0_QuoteOptionsDetailCmp extends LightningElement {
  //API
  @api recordId;
  @api objectApiName;
  ...
  wiredRecord({ error, data }) {
    if (data) {
     ...
     if (this.objectApiName == "SBQQ__Quote__c") {
        this.orderRecordType = data.fields.Quote_Record_Type_Text__c.value;
  ...
  setOrderRecordFields() {
    this.fields = [
      "SBQQ__Quote__c.Is_Edit_In_Progress__c",
      "SBQQ__Quote__c.RPP__c",
      "SBQQ__Quote__c.Apply_Standby_Rates__c",
      "SBQQ__Quote__c.Contingency_Quote__c",
      "SBQQ__Quote__c.Seasonal_Quote__c",
      "SBQQ__Quote__c.Quote_Record_Type_Text__c"
    ];
  }
  ...
  setEditVisibility() {
    if (this.objectApiName == "SBQQ__Quote__c") {
      if (
       // this.orderRecordType.includes("Rental Quote") &&
        this.isEditMode
      ) {
        //this.showEdit = true;
        this.isEdit = true;
  ...
  handleChange(event){
    console.log('we are at handlechange',event.target.value+this.recordId+event.target.fieldName)
    try{
