...
import getKey from '@salesforce/apex/SBR_3_0_FileUploadCmpController.getKey';
import encrypt from '@salesforce/apex/SBR_3_0_FileUploadCmpController.encrypt';
import createContentVers from '@salesforce/apex/SBR_3_0_FileUploadCmpController.createContentVers';
import appendDataToContentVersion from '@salesforce/apex/SBR_3_0_FileUploadCmpController.appendDataToContentVersion';
import createContentDocLink from '@salesforce/apex/SBR_3_0_FileUploadCmpController.createContentDocLink';
import deleteContentDoc from '@salesforce/apex/SBR_3_0_FileUploadCmpController.deleteContentDoc';
import getExistingFiles from '@salesforce/apex/SBR_3_0_FileUploadCmpController.getExistingFiles';
import updateFileName from '@salesforce/apex/SBR_3_0_FileUploadCmpController.updateFileName';

const MAX_FILE_SIZE = 4500000;
const CHUNK_SIZE = 750000;

export default class Sbr_3_0_FileUploadCmp extends NavigationMixin(LightningElement) {
    @api acceptedFormats;
    @api allowMultiple;
    ...
    validate() {
        if (this.docIds.length === 0 && this.required === true) {
            ...
            this.showToast({title: 'ERROR', msg: 'Upload at least 1 file to close the Work Order' , variant: 'error'});
            return {
                isValid: false,
                errorMessage: ''
            };
    ...
    handleUploadFinished(files) {
        ...
        if (this.recordId) {
            createContentDocLink({ versIds: versIds, encodedKey: this.key, visibleToAllUsers: this.visibleToAllUsers })
                .then(() => {
                    this.showToast({title: 'SUCCESS', msg: 'Files uploaded successfully.' , variant: 'success'});
                })
                .catch(error => {
                    this.showErrors(this.reduceErrors(error).toString());
                });
    ...
    deleteDocument(event) {
        ...
        if (this.disableDelete) {
            ...
            deleteContentDoc({ docId: documentId })
                .then(() => {
                    this.removeFileFromUi(contentVersionId);
                })
                .catch((error) => {
    ...
    showErrors(errors) {
        if (this.embedExternally) {
            ...
            this.showToast({title: 'We hit a snag.', msg: errors , variant: 'error'});
        }
    }
