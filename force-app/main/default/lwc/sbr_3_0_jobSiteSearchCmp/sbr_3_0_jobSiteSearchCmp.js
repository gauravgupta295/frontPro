...
export default class Lookupchild extends NavigationMixin(LightningElement) {
  // Public properties
  ...
  set errors(errors) {
    ...
    if (this._errors?.length > 0) {
      this.blur();
      this.errorList = errors.filter((item) => item !== "");
    }
  }
  ...
  processSelectionUpdate(isUserInteraction) {
    ...
    if (isUserInteraction) {
      ...
      this.checkValidity(selectedIds); // FRONT-4333
    }
  }
  ...
  handleBlur(event) {
    ...
    this.checkValidity(event.target.value); // FRONT-4333
  ...
  // FRONT-4333 start
  @api checkValidity(value) {
    if (this._curSelection.length > 0 || value) {
      this.hasJobSiteBlank = false;
      //  this._isDirty = false;
    } else {
      this.hasJobSiteBlank = true;
      // this._isDirty = true;
    }
  }
  // FRONT-4333 end
  ...
  handleNewRecordClick(event) {
    //Front-9257
    const jobSiteCreateEvent = new CustomEvent("jobsitecreation");
    this.dispatchEvent(jobSiteCreateEvent);
  ...
  get getInputClass() {
    ...
    if (
      ...
      css += "slds-has-error has-custom-error ";
    }
    if (!this.isMultiEntry) {
      css +=
  ...
  get getInputValue() {
    ...
    /*if (!this.hasSelection()) {
      this.applyBorderColor('#C23934');
    } else {
      this.applyBorderColor('grey');
    }*/
  ...
  applyBorderColor(colorVal) {
    const searchInputField = this.template.querySelector(
      `input[role="textbox"]`
    );
    if (searchInputField) {
      searchInputField.style.borderColor = colorVal;
      searchInputField.style.borderSize = "2px";
    }
  }
}
